...
function addComputer() {
	if (!change) {
		const newComputer = {
			id: myComputers.length === 0 ? 1 + currentUser : parseInt(myComputers.at(-1).id) + 1 + currentUser,
		...
		const index = myComputers.findIndex((comp) => comp.id + 'c' === id);
...
function deleteComputer(id) {
	myComputers = myComputers.filter((comp) => comp.id + 'd' !== id);
...
function changeComputer(id) {
	const currentComp = myComputers.find((comp) => comp.id + 'c' === id);
